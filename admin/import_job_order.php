...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['action'] ?? '') === 'save_import') {
        $order = collect_job_order_input($_POST, true);
        ...
        if (!$errors) {
            $jobOrderId = create_job_order($pdo, (int) current_user()['id'], $order, $chosenCategoryIds, $otherText);
            update_job_order_as_admin($pdo, $jobOrderId, (int) current_user()['id'], $order, $chosenCategoryIds, $otherText);
            redirect('/job-order-system/admin/view_job_order?id=' . $jobOrderId);
        }
    } else {
...
?>
<h1>Admin Import Job Order</h1>
<section class="panel">
    <form method="post" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <p class="muted">Vision, Textract, Tesseract, or another OCR service should feed a Pending Review preview before saving.</p>
...
<?php if ($draft): ?>
    <section class="panel">
        ...
        <p class="muted">Correct the fields below before saving. OCR is a draft only and this admin workflow stays on admin routes.</p>
    </section>
    <form method="post">
        <?= csrf_field() ?>
